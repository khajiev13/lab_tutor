...
class AttentionGCNLayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, n_heads: int = 4,
                 neg_slope: float = 0.2, dropout: float = 0.1):
        super().__init__()
    ...
    def forward(self, H: torch.Tensor, A: torch.Tensor) -> torch.Tensor:
        ...
        e    = e.masked_fill(~mask.unsqueeze(1).expand(-1, self.n_heads, -1), float("-inf"))
...
class BipartiteGCNStack(nn.Module):
    def __init__(self, d: int, n_targets: int, n_layers: int = 2, dropout: float = 0.1):
        ...
        self.fwd_layers = nn.ModuleList([BipartiteGCNLayer(d, d) for _ in range(n_layers)])
        self.bwd_layers = nn.ModuleList([BipartiteGCNLayer(d, d) for _ in range(n_layers)])
        self.bn   = nn.ModuleList([nn.BatchNorm1d(d) for _ in range(n_layers)])
        self.drop = nn.Dropout(dropout)

    def forward(self, H_src: torch.Tensor, A: torch.Tensor) -> torch.Tensor:
        H_tgt = self.target_emb
        for fwd, bwd, bn in zip(self.fwd_layers, self.bwd_layers, self.bn, strict=False):
...
class DomainGCN(nn.Module):
    ...
    def forward(self, H_dom: torch.Tensor, A_dom: torch.Tensor,
                A_ds: torch.Tensor) -> torch.Tensor:
        h = self.attn(self.basic(H_dom, A_dom), A_dom)
        return self.d2s(self.ln(h + self.proj(H_dom)), A_ds)


class MultiRelationalGCN(nn.Module):
    def __init__(self, d_skill_embed: int, d_domain_embed: int, d: int,
                 n_skills: int, n_questions: int, n_students: int,
        ...
        self.question_gcn = BipartiteGCNStack(d, n_questions, n_layers=n_layers, dropout=dropout)
        self.student_gcn  = SparseStudentGCN(d, n_students, dropout=dropout)

    def forward(self, H_s: torch.Tensor, H_d: torch.Tensor,
                A_dom: torch.Tensor, A_ds: torch.Tensor,
                A_pre: torch.Tensor, A_qs: torch.Tensor,
                A_uq: torch.Tensor) -> dict[str, torch.Tensor]:
...
class InteractionEncoder(nn.Module):
    ...
    def forward(self, q_idx: torch.Tensor, outcomes: torch.Tensor,
                timestamps: torch.Tensor, h_q: torch.Tensor) -> torch.Tensor:
...
class TBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, timestamps: torch.Tensor,
                dv: torch.Tensor, pm: torch.Tensor | None) -> torch.Tensor:
...
class TemporalModel(nn.Module):
    def __init__(self, d: int, n_heads: int, d_ff: int,
        ...
        self.blocks = nn.ModuleList([TBlock(d, n_heads, d_ff, dropout) for _ in range(n_layers)])
        self.ln_out = nn.LayerNorm(d)

    def forward(self, q_idx: torch.Tensor, outcomes: torch.Tensor,
                timestamps: torch.Tensor, dv: torch.Tensor,
                pm: torch.Tensor, h_q: torch.Tensor) -> torch.Tensor:
...
class ARCDModelPTADisc(nn.Module):
    ...
    def __init__(
        ...
        self.gcn = MultiRelationalGCN(
            d_skill_embed, d_domain_embed, d, n_skills, n_questions,
            n_students, n_layers=n_gcn_layers, dropout=dropout,
        )
        self.temporal = TemporalModel(d, n_heads, d_ff, n_attn_layers, dropout)
        self.response_head = nn.Sequential(
            nn.Linear(d * 3, d * 2), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(d * 2, d),    nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(d, 1),
        )
        self.mastery_head = nn.Sequential(
            nn.Linear(d, d), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(d, n_skills), nn.Sigmoid(),
        )

    def run_gcn(self, gd: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        out = self.gcn(
            gd["H_skill"], gd["H_domain"], gd["A_domain"],
            gd["A_ds"], gd["A_skill"], gd["A_qs"], gd["A_uq"],
        )
