...
def _post(url: str, payload: dict) -> tuple[dict, float]:
    ...
    resp = requests.post(
        url, json=payload, timeout=30, proxies={"http": None, "https": None}
    )
...
def run(base_url: str, data_dir: Path, n_students: int, history_frac: float) -> None:
    ...
    print(
        f"   status={health['status']}  checkpoint_loaded={health['checkpoint_loaded']}"
    )
    print(
        f"   model_version={health['model_version']}  best_val_auc={health['best_val_auc']:.4f}"
    )
    ...
    print(
        f"   n_skills={info['n_skills']}  n_questions={info['n_questions']}  "
    ...
    user_col = (
        ...
        else ("student_idx" if "student_idx" in df.columns else "student_id")
    )
    ...
    for uid in rng_users:
        rows = df[df[user_col] == uid].sort_values(
            ts_col if ts_col in df.columns else q_col
        )
        ...
        interactions_raw = [
            {
                ...
                "timestamp_sec": float(row[ts_col])
        ...
        try:
            ...
            p_map = {
                d["question_name"]: float(d["p_correct"])
